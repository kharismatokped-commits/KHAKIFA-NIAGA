...
export const ProductCard: React.FC<{ product: Product }> = ({ product }) => {
  ...
  const lowestPcsPrice = product.tieredPricesPcs.length > 0
    ? product.tieredPricesPcs[product.tieredPricesPcs.length - 1].price
    : 0;

  const highestPcsPrice = product.tieredPricesPcs.length > 0
    ? product.tieredPricesPcs[0].price
    : 0;
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200/90 hover:border-[#146C43] p-3 sm:p-4 shadow-xs hover:shadow-md transition-all duration-200">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
        ...
        >
          ...
          <div className="relative w-24 h-24 sm:w-28 sm:h-28 rounded-xl bg-gray-50 overflow-hidden shrink-0 border border-gray-100">
            ...
            {product.promoTag && (
              <div className="absolute top-1.5 left-1.5 z-10">
                <Badge variant="promo" className="text-[9px] px-1.5 py-0.2 font-extrabold uppercase shadow-xs">
                  {product.promoTag}
                </Badge>
              </div>
            )}
          ...
          <div className="flex-1 min-w-0">
            ...
            <h3 className="font-heading font-medium text-gray-900 text-[13px] sm:text-[14px] leading-snug line-clamp-2 group-hover:text-[#146C43] transition-colors">
              {product.name}
            </h3>

            {/* Harga Produk 16px bold (elemen paling menonjol) */}
            <div className="mt-1.5 flex flex-wrap items-baseline gap-1.5">
              <span className="text-[12px] text-gray-500 font-normal">Mulai:</span>
              <span className="font-price font-bold text-[#146C43] text-[16px] tracking-tight">
                {formatRupiah(lowestPcsPrice)}
              </span>
              <span className="text-[12px] text-gray-500 font-normal">/{product.unitPcsName}</span>
            ...
            {product.hasPack && (
              <div className="mt-1 flex items-center gap-1 text-[12px] text-emerald-800 font-medium">
                <Package className="w-3.5 h-3.5 text-[#146C43]" strokeWidth={2} />
                <span>Tersedia Grosir {product.unitPackName} (Isi {product.packRatio} pcs)</span>
              </div>
            )}
          </div>
        </Link>
        ...
        <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-center gap-2 pt-2 sm:pt-0 border-t sm:border-t-0 border-gray-100 shrink-0">
          <Link
            href={`/produk/${product.id}`}
            className="text-[12px] font-medium text-[#146C43] hover:underline flex items-center gap-0.5 sm:mb-1 order-2 sm:order-1"
          >
          ...
          <Button
            ...
            className={`min-h-[44px] h-11 px-5 rounded-xl font-heading font-medium text-[13px] sm:text-[14px] tracking-normal transition-all shadow-sm order-1 sm:order-2 w-full sm:w-auto ${
              added
                ? "bg-[#115b38] text-white ring-2 ring-emerald-300"
                : "bg-[#146C43] hover:bg-[#115b38] text-white"
            }`}
          >
            {added ? (
              <>
